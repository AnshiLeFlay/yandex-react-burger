...
function BurgerConstructor() {
    ...
    const burgerBun = useSelector(
        (store) => store.ingredients.burgerIngredients.bun
    );
    const burgerContent = useSelector(
        (store) => store.ingredients.burgerIngredients.consist
    );
    ...
    useEffect(() => {
        if (data[0] !== undefined) {
            ...
            orderArr.forEach((elem) => {
                cost += data.find((item: TIngredient) => item._id === elem)!
                    .price!;
            });
    ...
    return (
        <DropTarget>
            <div
                id="dropConstructor"
                className={`mt-25 pl-4 pr-4 ${styles.constructor_wrapper}`}
            >
                {bunTopBot?.image !== undefined && (
                    <BurgerConstructorItemWrapper
                        type="bun"
                        pos="top"
                        key={"top." + bunTopBot?._id}
                        image={bunTopBot?.image!}
                        price={bunTopBot?.price!}
                        name={bunTopBot?.name + " (верх)"}
                    />
                )}

                {constructor.map((elem: any, index: number) => (
                    <DraggableItem
                        key={"key_constructor_" + index + "." + elem._id}
                        data={{
                            id: elem._id,
                            content: "",
                            board: "constructor",
                    ...
                    >
                        <BurgerConstructorItemWrapper
                            key={"constructor." + index + "." + elem._id}
                            position={index}
                            image={elem.image}
                            price={elem.price}
                            name={elem.name}
                ...
                {bunTopBot?.image !== undefined && (
                    <BurgerConstructorItemWrapper
                        type="bun"
                        pos="bottom"
                        key={"bot." + bunTopBot._id}
                        image={bunTopBot.image}
                        price={bunTopBot?.price!}
                        name={bunTopBot?.name + " (низ)"}
                    />
                )}
